...
//	}
			...
			if(mouse.pressed==false) 
			{
				if(activeP !=null)
				{
					...
					//update the pieces list in the case a piece has been captured and removed during the simulation
						...
						//move is not valid so it will reset everything
					copyPieces(pieces,simPieces);
	...
	private boolean isIllegal(Piece king) {
		if(king.type==Type.KING) {
			for(Piece piece:simPieces) {
				if(piece!= king && piece.color !=king.color && piece.canMove(king.col, king.row)) { //next move king is getting checked
					return true;
				}
			}
		}
	...
	private boolean isCheckmate() {
		...
		if(kingCanMove(king)) {
			...
			if(colDiff==0) {
				...
				//The check from horizontal
				if(checkingP.col<king.col) {//left side se
				...
				if(checkingP.col>king.col) {//right side se
					for(int col=checkingP.col;col>king.col;col--) {
						for(Piece piece:simPieces) {
							if(piece !=king && piece.color !=currentColor && piece.canMove(checkingP.col, col)) {
							return false;	
				...
				//knights attack cant be blocked
	...
	private boolean kingCanMove(Piece king) {
		//simulate if there is an any square left for the king to move aka raja ghumavo
	...
	private boolean isValidMove(Piece king,int colPlus,int rowPlus) {
		...
		king.col+=colPlus;
		king.row+=rowPlus;
		//if king can hit a piece if the move is not illegal detecting
		...
		}//reset position and remove the piece
		king.resetPosition();
		copyPieces(pieces,simPieces);
	...
	private void changePlayer()
	{
		...
		{
			currentColor=BLACK;
			//RESET karna hai black pawn ka  2 step so enpass hojai 
			...
			currentColor=WHITE;
			//RESET karna hai white pawn ka  2 step so enpass hojai
			for(Piece piece:pieces) {
	...
	if (gameover) {
	    String s = (currentColor == WHITE) ? "Checkmate! White Wins" : "Checkmate! Black Wins";

	    // Set the font and calculate dimensions
	    ...
	    int centerY = (getHeight() / 2) + (textHeight / 4); // Adjust Y for vertical centering

	    // Draw background rectangle
	    int padding = 20;
	    g2.setColor(new Color(0, 0, 0, 180)); // Semi-transparent black
	    g2.fillRoundRect(centerX - padding, centerY - textHeight - padding, textWidth + 2 * padding, textHeight + 2 * padding, 50, 50);

	    // Draw text with a stylish color
	    g2.setColor(Color.GREEN);
	    g2.drawString(s, centerX, centerY);

	    // Optional: Add shadow effect
	    g2.setColor(new Color(0, 0, 0, 150));
	    g2.drawString(s, centerX + 4, centerY + 4);
	}
	if (stalemate) {
	    String s = "Stalemate! It's a Draw";
